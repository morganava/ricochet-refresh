    ...
    pub fn write_to_vec(&self, v: &mut Vec<u8>) -> Result<(), Error> {
        ...
        match self {
            Packet::FileHeader(file_header) => {
                ...
                let file_header = protos::FileChannel::FileHeader {
                    file_id,
                    file_size,
                    name,
                    file_hash,
            ...
            Packet::FileHeaderAck(file_header_ack) => {
                ...
                let file_header_ack = protos::FileChannel::FileHeaderAck {
                    file_id,
                    accepted,
                    ..Default::default()
                };
            ...
            Packet::FileHeaderResponse(file_header_response) => {
                ...
                let file_header_response = protos::FileChannel::FileHeaderResponse {
                    file_id,
                    response,
                    ..Default::default()
                };
            ...
            Packet::FileChunk(file_chunk) => {
                ...
                let file_chunk = protos::FileChannel::FileChunk {
                    file_id,
                    chunk_data,
                    ..Default::default()
                };
            ...
            Packet::FileChunkAck(file_chunk_ack) => {
                ...
                let file_chunk_ack = protos::FileChannel::FileChunkAck {
                    file_id,
                    bytes_received,
                    ..Default::default()
                };
            ...
            Packet::FileTransferCompleteNotification(file_transfer_complete_notification) => {
                ...
                let result = match result {
                    ...
                    FileTransferResult::Cancelled => {
                        protos::FileChannel::FileTransferResult::Cancelled
                    }
                };
                let result = Some(protobuf::EnumOrUnknown::new(result));

                let file_transfer_complete_notification =
                    protos::FileChannel::FileTransferCompleteNotification {
                        file_id,
                        result,
                        ..Default::default()
                    };

                pb.file_transfer_complete_notification =
                    Some(file_transfer_complete_notification).into();
            }
        }
...
impl TryFrom<&[u8]> for Packet {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        let pb = protos::FileChannel::Packet::parse_from_bytes(value)
        ...
        let file_transfer_complete_notification =
            pb.file_transfer_complete_notification.into_option();
        ...
        match (
            ...
            (Some(file_header), None, None, None, None, None) => {
                let file_id = file_header
                    .file_id
                    .ok_or(Self::Error::InvalidProtobufMessage)?;

                let file_size = file_header
                    .file_size
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
            ...
            (None, Some(file_header_ack), None, None, None, None) => {
                let file_id = file_header_ack
                    .file_id
                    .ok_or(Self::Error::InvalidProtobufMessage)?;

                let accepted = file_header_ack
                    .accepted
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
            ...
            (None, None, Some(file_header_response), None, None, None) => {
                let file_id = file_header_response
                    .file_id
                    .ok_or(Self::Error::InvalidProtobufMessage)?;

                let response = file_header_response
                    .response
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
            ...
            (None, None, None, Some(file_chunk), None, None) => {
                let file_id = file_chunk
                    .file_id
                    .ok_or(Self::Error::InvalidProtobufMessage)?;

                let chunk_data = file_chunk
                    .chunk_data
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
            ...
            (None, None, None, None, Some(file_chunk_ack), None) => {
                let file_id = file_chunk_ack
                    .file_id
                    .ok_or(Self::Error::InvalidProtobufMessage)?;

                let bytes_received = file_chunk_ack
                    .bytes_received
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
            ...
            (None, None, None, None, None, Some(file_transfer_complete_notification)) => {
                let file_id = file_transfer_complete_notification
                    .file_id
                    .ok_or(Self::Error::InvalidProtobufMessage)?;

                let result = file_transfer_complete_notification
                    .result
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
                ...
                let file_transfer_complete_notification =
                    FileTransferCompleteNotification::new(file_id, result)?;
                Ok(Packet::FileTransferCompleteNotification(
                    file_transfer_complete_notification,
                ))
            }
    ...
    pub fn new(
        ...
        if name.contains("..") || name.contains("/") {
            Err(Error::PacketConstructionFailed(
                "name contains forbidden substring".to_string(),
            ))
        } else {
            Ok(Self {
                file_id,
...
impl TryFrom<i32> for Response {
    ...
    fn try_from(value: i32) -> Result<Response, Self::Error> {
        match value {
            ...
            _ => Err(Self::Error::PacketConstructionFailed(
                "response must be 0 or 1".to_string(),
            )),
        }
    }
}
...
impl ChunkData {
    pub fn new(data: Vec<u8>) -> Result<ChunkData, Error> {
        ...
        if data_len > MAX_FILE_CHUNK_SIZE {
            Err(Error::PacketConstructionFailed(format!(
                "chunk data must be less than {MAX_FILE_CHUNK_SIZE} bytes"
            )))
        } else {
            Ok(Self { data })
        }
