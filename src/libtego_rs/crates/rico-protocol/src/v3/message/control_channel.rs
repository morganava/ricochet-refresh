    ...
    pub fn write_to_vec(&self, v: &mut Vec<u8>) -> Result<(), Error> {
        ...
        match self {
            Packet::OpenChannel(open_channel) => {
                ...
                let mut open_channel = protos::ControlChannel::OpenChannel {
                    channel_identifier,
                    channel_type,
                    ..Default::default()
                };
                ...
                match extension {
                    Some(OpenChannelExtension::ContactRequestChannel(extension)) => {
                        ...
                        let contact_request = protos::ContactRequestChannel::ContactRequest {
                            nickname,
                            message_text,
                            ..Default::default()
                        };
                        ...
                        open_channel.mut_unknown_fields().add_length_delimited(
                            field_number,
                            contact_request.write_to_bytes().unwrap(),
                        );
                    }
                    Some(OpenChannelExtension::AuthHiddenService(extension)) => {
                        ...
                        let field_number =
                            crate::auth_hidden_service::OpenChannel::CLIENT_COOKIE_FIELD_NUMBER;

                        open_channel
                            .mut_unknown_fields()
                            .add_length_delimited(field_number, client_cookie.into());
                    }
                    None => (),
                }
            ...
            Packet::ChannelResult(channel_result) => {
                ...
                let common_error = if let Some(common_error) = common_error {
                    let common_error = match common_error {
                        CommonError::GenericError => {
                            protos::ControlChannel::channel_result::CommonError::GenericError
                        }
                        CommonError::UnknownTypeError => {
                            protos::ControlChannel::channel_result::CommonError::UnknownTypeError
                        }
                        CommonError::UnauthorizedError => {
                            protos::ControlChannel::channel_result::CommonError::UnauthorizedError
                        }
                        CommonError::BadUsageError => {
                            protos::ControlChannel::channel_result::CommonError::BadUsageError
                        }
                        CommonError::FailedError => {
                            protos::ControlChannel::channel_result::CommonError::FailedError
                        }
                    };
                ...
                let mut channel_result = protos::ControlChannel::ChannelResult {
                    channel_identifier,
                    opened,
                    common_error,
                    ..Default::default()
                ...
                match extension {
                    Some(ChannelResultExtension::ContactRequestChannel(extension)) => {
                        ...
                        let status = match status {
                            crate::contact_request_channel::Status::Undefined => {
                                protos::ContactRequestChannel::response::Status::Undefined
                            }
                            crate::contact_request_channel::Status::Pending => {
                                protos::ContactRequestChannel::response::Status::Pending
                            }
                            crate::contact_request_channel::Status::Accepted => {
                                protos::ContactRequestChannel::response::Status::Accepted
                            }
                            crate::contact_request_channel::Status::Rejected => {
                                protos::ContactRequestChannel::response::Status::Rejected
                            }
                            crate::contact_request_channel::Status::Error => {
                                protos::ContactRequestChannel::response::Status::Error
                            }
                        };
                        let status = Some(protobuf::EnumOrUnknown::new(status));

                        let response = protos::ContactRequestChannel::Response {
                            status,
                            ..Default::default()
                        };

                        let field_number =
                            crate::contact_request_channel::ChannelResult::RESPONSE_FIELD_NUMBER;

                        channel_result
                            .mut_unknown_fields()
                            .add_length_delimited(field_number, response.write_to_bytes().unwrap());
                    }
                    Some(ChannelResultExtension::AuthHiddenService(extension)) => {
                        let server_cookie = &extension.server_cookie;

                        let field_number =
                            crate::auth_hidden_service::ChannelResult::SERVER_COOKIE_FIELD_NUMBER;

                        channel_result
                            .mut_unknown_fields()
                            .add_length_delimited(field_number, server_cookie.into());
                    }
                    None => (),
                }
...
impl TryFrom<&[u8]> for Packet {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        let pb = protos::ControlChannel::Packet::parse_from_bytes(value)
        ...
        match (
            ...
            (Some(open_channel), None, None, None, None) => {
                // base fields
                let channel_identifier = open_channel
                    .channel_identifier
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
                let channel_type = open_channel
                    .channel_type
                    .as_ref()
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
                ...
                // extension fields
                let contact_request =
                    protos::ContactRequestChannel::exts::contact_request.get(&open_channel);
                let client_cookie =
                    protos::AuthHiddenService::exts::client_cookie.get(&open_channel);
                ...
                let extension: Option<OpenChannelExtension> = match channel_type {
                    ...
                    ChannelType::ContactRequest => {
                        let mut contact_request =
                            contact_request.ok_or(Self::Error::InvalidProtobufMessage)?;
                        if client_cookie.is_some() {
                            return Err(Self::Error::InvalidProtobufMessage);
                        }

                        let nickname = contact_request.take_nickname();
                        let nickname: crate::contact_request_channel::Nickname =
                            nickname.try_into()?;

                        let message_text = contact_request.take_message_text();
                        let message_text: crate::contact_request_channel::MessageText =
                            message_text.try_into()?;

                        let contact_request = crate::contact_request_channel::ContactRequest {
                            nickname,
                            message_text,
                        };

                        Some(OpenChannelExtension::ContactRequestChannel(
                            crate::contact_request_channel::OpenChannel { contact_request },
                        ))
                    }
                    ...
                    ChannelType::AuthHiddenService => {
                        ...
                        let client_cookie =
                            client_cookie.ok_or(Self::Error::InvalidProtobufMessage)?;

                        let client_cookie: [u8; crate::auth_hidden_service::CLIENT_COOKIE_SIZE] =
                            match client_cookie.try_into() {
                        ...
                        Some(OpenChannelExtension::AuthHiddenService(
                            crate::auth_hidden_service::OpenChannel { client_cookie },
                        ))
                    }
                    _ => None,
                };
            ...
            (None, Some(channel_result), None, None, None) => {
                // base fields
                let channel_identifier = channel_result
                    .channel_identifier
                    .ok_or(Self::Error::InvalidProtobufMessage)?;

                let opened = channel_result
                    .opened
                    .ok_or(Self::Error::InvalidProtobufMessage)?;
                ...
                let server_cookie =
                    protos::AuthHiddenService::exts::server_cookie.get(&channel_result);
                ...
                let extension: Option<ChannelResultExtension> = match (response, server_cookie) {
                    ...
                    (Some(response), None) => {
                        ...
                        Some(ChannelResultExtension::ContactRequestChannel(
                            crate::contact_request_channel::ChannelResult { response },
                        ))
                    }
                    // auth hidden service channel result
                    (None, Some(server_cookie)) => {
                        let server_cookie: [u8; crate::auth_hidden_service::SERVER_COOKIE_SIZE] =
                            match server_cookie.try_into() {
                                Ok(server_cookie) => server_cookie,
                                Err(_) => return Err(Self::Error::InvalidProtobufMessage),
                            };

                        Some(ChannelResultExtension::AuthHiddenService(
                            crate::auth_hidden_service::ChannelResult { server_cookie },
                        ))
                    }
                    (None, None) => None,
                    _ => return Err(Self::Error::InvalidProtobufMessage),
                ...
                let channel_result =
                    ChannelResult::new(channel_identifier, opened, common_error, extension)?;
...
impl OpenChannel {
    pub fn new(
        ...
        if channel_identifier < 1 || channel_identifier > u16::MAX as i32 {
            Err(Error::PacketConstructionFailed(
                "channel_identifier must be postive, non-zero, and less than u16::MAX".to_string(),
            ))
        } else {
...
impl ChannelResult {
    pub fn new(
        ...
        if channel_identifier < 1 || channel_identifier > u16::MAX as i32 {
            Err(Error::PacketConstructionFailed(
                "channel_identifier must be postive, non-zero, and less than or equal to u16::MAX"
                    .to_string(),
            ))
        } else {
            let channel_identifier = channel_identifier as u16;
