...
| Columns |
|------- |
| conversation_rowid, user_rowid, message_sequence, record_sequence |
| conversation_rowid, user_rowid, create_timestamp, message_sequence, record_sequence |
| conversation_rowid, create_timestamp DESC, record_sequence DESC |
...
- **modify_timestamp** : the UTC timestamp of when this message was most recently modified; if not modified same value as `create_timestamp`
    - milliseconds since the UNIX epoch
...
- `modify_timestamp` : the 8-byte big-endian `modify_timestamp` value for this `message_record`
